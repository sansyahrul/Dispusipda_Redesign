...
export default function StatistikImportPage() {
  ...
  const [data, setData] = useState<StatistikImport[]>([]);
  ...
  const handleUpload = async (
    ...
    try {
      const res = await fetch("/api/Statistik/import", {
        method: "POST",
        body: formData,
      });
  ...
  const handleClearData = async () => {
    ...
    try {
      const res = await fetch("/api/Statistik/clear", { method: "DELETE" });
      const result = await res.json();
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-8">
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-6xl mx-auto mb-12">
          ...
          >
            <div className="dashboard-title flex items-center gap-3 mb-3">
            ...
            <p className="text-gray-600 text-sm leading-relaxed mb-4 text-center mt-15">
              Input total data koleksi per tahun untuk Karya Cetak dan Karya
              Rekam. Data akan ditampilkan
            </p>
            <p className="text-gray-600 text-sm leading-relaxed mb-4 text-center mt-8">
              dalam grafik batang untuk perbandingan antar tahun.
            </p>

            <input
              type="file"
              accept=".xlsx,.xls"
              onChange={(e) => handleUpload(e, "koleksi")}
              className="hidden"
              id="upload-koleksi"
              disabled={isLoading}
            />
        ...
        <div className="bg-white rounded-2xl border border-[#C9D4E1] shadow-sm overflow-hidden">
          ...
          {/* 📊 CHART */}
          <div
            className="mt-6 border border-slate-200 rounded-[25px] shadow-md p-6"
            style={{
              backgroundColor: "#F9FCFF",
              borderColor: "#C9D4E1",
            }}
          >
            <h3 className="font-semibold text-slate-700 mb-4 text-center">
              Grafik Pertumbuhan Koleksi
            </h3>
            <div className="w-full h-80">
              {chartData.length > 0 ? (
                <StatistikChart data={chartData} />
              ) : (
